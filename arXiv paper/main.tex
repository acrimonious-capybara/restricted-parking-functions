...
In fact, this generalizes to a way to enumerate $S$-restricted prime parking functions as $T$-restricted parking functions for some related subset $T$. We prove this more general statement, from which \cref{thm:primeIsRes} follows:
...
\begin{proof}z
	Imagine a circular parking lot with $gs$ spots and only permit cars to prefer the $s$ spots $1, g + 1, 2g + 1, \dots$ in $S$. The $gs - 1$ cars perform the regular parking procedure --- they drive to their preferred spot, attempt to park there, and then each subsequent spot. However, in this parking lot if cars are unable to park in a spot after their preferred spot, they can circle back to the spots before their preference. Thus, all cars are always able to park.
	...
	The preference functions on the circle also have circular symmetry. If $\pi$ leads to car $i$ parking in spot $\pi^{i}$ and $\psi = \pi + a \pmod{gs}$, then $\psi$ results in each car parking in spot $\psi^{i} = \pi^{i} + \pmod{gs}$. \todo[jayden]{explain why, explain the orbits} In particular, there is a natural action of the cyclic group $\mathbb{Z} / s \mathbb{Z}$ by $x \cdot \pi = \pi + xg \pmod{gs}$. Each orbit of this action contains exactly one parking function with spot $gs$ empty --- for $\pi$ with spot $g \sigma$ empty, $(s - \sigma) \cdot \pi$ has spot $gs$ empty.
