...
\begin{definition}
    An $S$-restricted parking function on $n$ cars, given a set $S,$ is a parking function on $n$ cars where all preferences lie in $S.$ We denote the number of parking functions of this type as $\# \mathrm{PF}_{n\mid S}.$
\end{definition}

Note that since every parking function has at least one preference equal to $1,$
