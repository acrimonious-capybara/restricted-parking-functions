...
\section{Introduction} %\todo[jayden]{make this more friendly to someone who is coming from polytopes or with less background in PFs}

Take $n$ cars for some $n\in\mathbb{N},$ all of which are attempting to park along a one-way street with a total of $n$ spots. Each car has a preferred spot, as described by a function $\pi:[n]\to[n],$ where the $i$th car prefers the $\pi(i)$th spot. We sometimes describe this as a preference list of length $n$ instead, where the $i$th element is the preference of the $i$th car.
...
\end{enumerate}
We will show that all of these can be understood using \emph{$S$-restricted parking functions} --- parking functions $\pi : [n] \to S \subseteq [n]$.

\begin{definition}
    An $S$-restricted parking function on $n$ cars, given a set of possible preferences $S$ is a parking function $\pi$ on $n$ cars where all preferences lie in $S$: that is, $\pi([n]) \subset S$.

    We denote the set of $S$-restricted parking functions by $\mathrm{PF}_{n \mid S},$ and therefore their number by $\# \mathrm{PF}_{n \mid S}$.
\end{definition}

These reinterpretations yield new combinatorial insights, simpler proofs, more complete theories of parking procedures, and connections to deep results in combinatorics. In some sense, $S$-restricted parking functions provide a paradigm for solving parking questions that strikes the balance between the generality of $\mathbf{u}$-parking functions --- of which they are the special case $\mathbf{u} = ( \# [1] \cap S , \dots, \# [n] \cap S)$ --- and the combinatorial intuition of the original parking function question.
...
\begin{definition}[prime parking functions]
	A \textit{prime parking function} on $n$ cars is a function describing the preferences of $n$ cars on a street with $n$ cars, $\pi : [n] \to [n],$ such that more than $i$ cars prefer the first $i$ spots (for $1 \le i \le n$). In other words $\# \pi^{-1}([i]) > i$.
\end{definition}
