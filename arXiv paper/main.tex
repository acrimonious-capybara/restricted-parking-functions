...
Some choices of $\pi$ lead to all $n$ cars parking; for example, any permutation $\pi\in S_n$ lets each car park in its preferred spot, while if every car prefers the first spot, the street fills up from beginning to end. However, not all choices of $\pi$ will lead to this result; for example, if no car prefers spot 1, then the first spot will never fill, so that not all cars can park in the remaining $n-1$ spots.

The \emph{parking functions} on $n$ cars are those $\pi : [n] \to [n]$ which take each of $n$ cars to their preferences in a manner that allows all cars to park; their theory is well understood. For instance, they have been enumerated precisely; there are $(n + 1)^{n - 1}$ parking functions on $n$ cars. Parking functions are characterized by the following \emph{Catalan condition}: a function $\pi : [n] \to [n]$ is a parking function if and only if $\#\pi^{-1}([i]) \ge i$ for each $i \in [n]$. That is, a function is a parking function if at least $i$ cars prefer to park in one of the first $i$ spots. This can also be written in the language of order-preserving parking functions. For each parking function, there is a unique \emph{order-preserving rearrangement} $\pi^{\uparrow}$ such that $\pi^{\uparrow} = \pi \circ \sigma$ for some permutation $\sigma \in \mathfrak{S}_{n}$. The Catalan condition can equivalently be stated: $\pi$ is a parking function if and only if $\pi(i) \le i$ for all $i \in [n]$.

There are many, many possible variants of the parking function question, some of which have gained traction in recent years; the interested reader is encouraged to peruse \cite{carlson-harris-2020} for an engaging overview of such variants. In many cases, these variants seem sufficiently different from classical parking functions that one cannot easily apply the body of theory which has been developed so far. We highlight in particular:
\begin{enumerate}
	\item Prime parking functions --- parking functions which one cannot break into smaller parking functions or alternatively functions which satisfy a stricter Catalan condition. See \cite{meyles-2023} for an excellent overview of their properties.
...
\begin{definition}
    Given a subset of allowed preferences $S \subseteq [n]$, an $S$-restricted parking function on $n$ cars is a parking function $\pi : [n] \to [n]$ where all preferences lie in $S$: that is, $\pi([n]) \subseteq S$.
...
\begin{definition}[prime parking functions]
	A \textit{prime parking function} on $n$ cars is a function describing the preferences of $n$ cars on a street with $n$ cars, $\pi : [n] \to [n],$ such that more than $i$ cars prefer the first $i$ spots (for $1 \le i \le n$). Equivalently, $\# \pi^{\uparrow}(i) < i$ for each $i \in [n]$. 

	We denote the set of all prime parking functions on $n$ cars by $\mathrm{PPF}_{n}$ and their number by $\# \mathrm{P P F}_{n}$.

\end{definition}

Prime parking functions themselves have a characterization in terms of $S$-restricted parking functions: prime parking functions are in bijection with parking functions where no car prefers the second spot. The bijection we present follows by reassigning cars' preferences in a  certain manner to turn a restricted parking function into a prime parking function.
...
In fact, this generalizes to a way to enumerate $S$-restricted prime parking functions as $T$-restricted parking functions for some related subset $T$. We prove this more general statement from which \cref{thm:primeIsRes} follows by a bijection that adds $1$ to all preferences greater than $1$. 
...
\begin{proof}
	Consider the natural bijection $f : S \setminus \{ n \} \to T$ that adds 1 to all $i \in S\backslash \{n\}$ greater than 1. That is, $f(1) = 1$ and $f(x) = x + 1$ otherwise. We claim this pushes forward to a bijection from $S$-restricted prime parking functions to $T$-restricted parking functions by $f_{*} : \pi \mapsto f_{*} \pi = f \circ \pi$.

	We first claim that $f_{*}$ is a well-defined map  $\mathrm{PPF}_{n \mid S}\to\mathrm{PF}_{n \mid T}$. Note first that no prime parking function can have $n$ in its image, since all $n$ preferences lie in $[n-1].$ If $\pi$ is an $S$-restricted prime parking function, it follows that $f \circ \pi$ is by definition a function $[n] \to T$. Since $\pi$ is prime, more than $i$ cars prefer a spot in the first $i$ for each $i$. For $f \circ \pi$, since $f(1) = 1$, this means that at least $2$ cars prefer one of the first $2$ spots (both prefer the first spot). For $i > 2$, all the cars $j$ with $\pi(j) \le i - 1$ now prefer spots in the first $i$. Since there are $k > i - 1$ and thus, $k \ge i$ of these, there are at least $i$ cars preferring spots in the first $i$. That is, $f_{*} \pi$ is a parking function.

	These equalities work in reverse too. For any $T$-restricted parking function $\psi$, $f^{-1} \circ \psi$ is a function $[n] \to S$. Note that $2\not\in T,$ which is required for $f^{-1}$ to be well-defined. Again $\psi$ must have $2$ cars prefer the first spot. For every other $i$th spot ($i > 1$), there are at least $i + 1$ cars $j$ such that $\psi(j) \le i + 1$ and thus, more than $i$ cars such that $(f^{-1} \circ \psi)(j) \le i$. Thus $f_{*}$ and $f_{*}^{-1}$ are both well-defined. Composing these functions in either direction gives the identity, so that $f_{*}$ is the desired bijection.
\end{proof}
...
One variant on classical parking functions occurs when when the number of cars differs from the number of spots to park in. \cite{cameron-johannsen-prellberg-schweitzer-2008} studies this idea in some depth. One particularly interesting case occurs when there are more cars than parking spots; not all cars will be able to park, but we can try to minimize the defect of the function: that is, the number of cars which are unable to park.

These minimum-defect parking functions are closely related to a particular class of preference-restricted parking functions:

\begin{theorem}
    Given $1\le s\le n,$ the number of ways for $n$ cars to park in $s$ spots, with the minimum possible defect of $n-s,$ is equal to $\# \mathrm{PF}_{n \mid [s]},$ where $[s]=\{1,2,\dots,s\}.$
\end{theorem}
