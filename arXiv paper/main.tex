...
\todo[jayden, inline]{add your author details in alphabetical order of last name --- \LaTeX \, doesn't do it automatically}
...
\]
which is given by Abel's theorem with $x=x$, $y=s-c-1$, $n=c$, and $i=i$. Notably, this is very nearly recovers the complete identity! In particular, our claim is proven for all values of $x$ and all integer values of $s$ from $1$ to $c$; since both sides of our identity are polynomials of degree $c$ in the variable $s$, only one further value of $s$ needs to be verified in order to show the full theorem as a consequence.
