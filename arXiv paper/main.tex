...
Both of these facts have analogues for $[s]$-restricted parking functions. \todo[jayden]{prove them}

\begin{theorem}
	Under the described action of $\mathfrak{S}_{n}$ on $\mathrm{PF}_{n \mid [s]}$, the number of orbits is $C(n, s - 1)$ orbits --- the $(n, s - 1)$th entry in Catalan's triangle, also known as the Narayana number $N(n, s - 1)$ \cite[A001263]{oeis}.
\end{theorem} 
